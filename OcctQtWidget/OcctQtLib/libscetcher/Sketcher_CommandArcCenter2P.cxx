...
IMPLEMENT_STANDARD_RTTIEXT(Sketcher_CommandArcCenter2P, Sketcher_Command)
 ...
 */
Standard_Boolean Sketcher_CommandArcCenter2P::MouseInputEvent(const gp_Pnt2d& thePnt2d, bool ortho)
{
...
//			myRubberLine->SetPoints(myFirstPoint,myFirstPoint);
			myContext->Display(myRubberCenterPoint,0,-1, false);
			myContext->Redisplay(myRubberCenterPoint, true);
	...
	case Input_2ArcPoint:
			...
			if (ProjectOnCircle(curPnt2d))
			{
				...
				if (tempGcc_Circ2d3Tan.IsDone() && tempGcc_Circ2d3Tan.NbSolutions()>0)
				{
					...
					myContext->Remove(myRubberCenterPoint, false);
 ...
 * \param thePnt2d const gp_Pnt2d&
 */
void Sketcher_CommandArcCenter2P::MouseMoveEvent (const gp_Pnt2d& thePnt2d, bool ortho)
{
 curPnt2d = myAnalyserSnap->MouseMove(thePnt2d);
 switch (myArcCenter2PAction)
 {
...
void Sketcher_CommandArcCenter2P::CancelEvent()
{
 switch (myArcCenter2PAction)
 {
	...
	case Input_1ArcPoint:	
	case Input_MidPoint:	myContext->Remove(myRubberLine, true);

	case Input_2ArcPoint:	myContext->Remove(myRubberCircle, false);
							myContext->Remove(myRubberCenterPoint, true);
							break;
	default: break;
 }
 myArcCenter2PAction = Nothing;
