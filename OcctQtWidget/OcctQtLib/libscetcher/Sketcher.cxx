...
/**
 * \fn SetOrtho(Sketcher_OrthoType theEnum)
 * \brief set ortho
 * \return void
 * \param theEnum Sketcher_OrthoType
 */
void Sketcher::SetOrtho(Sketcher_OrthoType theEnum){
    myOrtho=theEnum;
}
							    ...
							    const Standard_Real& projVx,const Standard_Real& projVy,const Standard_Real& projVz)
{
	...
	{
		SelectCurCommand();
        if(CurCommand->MouseInputEvent(myCurrentPnt2d,myOrtho))
			myCurrentMethod = Nothing_Method;
	}
}
							  ...
							  const Standard_Real& projVx,const Standard_Real& projVy,const Standard_Real& projVz)
{
	...
	{
		SelectCurCommand();
        CurCommand->MouseMoveEvent(myCurrentPnt2d,myOrtho);
	}
}
